...
Full rule: [`rules/my_powershell_download.yml`](rules/my_powershell_download.yml)
...
| Test | Expected after tuning | Result |
|---|---:|---:|
| `Write-Host "Hello World"` | No alert | Pass |
| `Get-Process` | No alert | Pass |
| `Invoke-WebRequest -Uri "https://example.com"` | No alert | Pass |
| `Invoke-WebRequest -Uri ... -OutFile ...` | Alert | Pass |
| `iwr -Uri ... -OutFile ...` | Alert | Pass |

See [`docs/testing-and-tuning.md`](docs/testing-and-tuning.md) for the full tuning rationale.

## Screenshots

### Real Event ID 4104 telemetry

![Real Event ID 4104 telemetry](screenshots/01-real-4104-events.png)

### Invoke-WebRequest captured in real telemetry

![Invoke-WebRequest 4104 event](screenshots/02-invoke-webrequest-event.png)

### Sigma-derived detection matching real telemetry

![Sigma detection match](screenshots/03-sigma-detection-match.png)
